    ...
    fetch('/games/list')
        ...
        .then(gameList => {
            ...
            gameList.forEach(gameId => {
                ...
                option.value = gameId;
                // A simple transformation for display name
                option.textContent = gameId.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
                gameModeSelect.appendChild(option);
            });
            // After populating, trigger change to load options for the first game
            gameModeSelect.dispatchEvent(new Event('change'));
        })
        .catch(error => {
            console.error('Error fetching game list:', error);
            gameModeSelect.innerHTML = '<option>Error loading games</option>';
        });
}
...
startGameButton.addEventListener('click', () => {
    ...
    urlParams.set('game', selectedGame);
    if (aiCommentary) {
        urlParams.set('aiCommentary', 'true');
    }
    for (const key in options) {
